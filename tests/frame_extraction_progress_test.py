...
def test_reporter_forwards_items_unchanged_and_emits_every_frame_for_small_video() -> None:
    ...
    reporter = make_progress_reporter(progress_queue=progress_queue, video_index=2, frame_total=5)
...
def test_reporter_throttles_updates_by_stride() -> None:
    ...
    reporter = make_progress_reporter(progress_queue=progress_queue, video_index=0, frame_total=300)
...
def test_reporter_stride_is_capped_at_max_frames_per_update() -> None:
    ...
    reporter = make_progress_reporter(progress_queue=progress_queue, video_index=1, frame_total=100_000)
...
def test_reporter_emits_on_final_frame_off_the_stride() -> None:
    ...
    reporter = make_progress_reporter(progress_queue=progress_queue, video_index=4, frame_total=201)
...
def test_reporter_suppresses_queue_errors_and_still_forwards_items() -> None:
    ...
    reporter = make_progress_reporter(progress_queue=raising_queue, video_index=0, frame_total=3)
...
def test_reporter_accepts_and_ignores_extra_tqdm_arguments() -> None:
    ...
    progress_queue: queue.Queue = queue.Queue()
    reporter = make_progress_reporter(progress_queue=progress_queue, video_index=0, frame_total=3)
...
def test_aggregate_bar_grand_total_clamped_to_one_for_empty_totals() -> None:
    ...
    bar = _make_bar(frame_totals={}, total_video_count=0)
...
def test_aggregate_bar_grand_total_sums_per_video_totals() -> None:
    ...
    bar = _make_bar(frame_totals={0: 100, 1: 250})
...
def test_ingest_progress_updates_frame_count_without_forcing_redraw() -> None:
    ...
    bar = _make_bar(frame_totals={3: 100})
...
def test_ingest_done_completes_known_video_and_forces_redraw() -> None:
    ...
    bar = _make_bar(frame_totals={1: 100})
...
def test_is_preparing_true_before_any_work() -> None:
    ...
    bar = _make_bar(frame_totals={0: 100})
...
def test_compose_preparing_shows_label_video_count_and_elapsed() -> None:
    ...
    bar = _make_bar(frame_totals={0: 100, 1: 100}, total_video_count=2)
...
def test_compose_active_clamps_frames_and_zero_active_when_all_done() -> None:
    ...
    bar = _make_bar(frame_totals={0: 100}, total_video_count=1)
...
class _RaisingQueue:
    """Records every ``put_nowait`` attempt and then raises, exercising the exception suppression path.
