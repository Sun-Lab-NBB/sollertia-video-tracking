"""Tests for the video-grouping algorithm that clusters related videos by their shared, non-date name components."""
...
def test_group_videos_infers_key_and_preserves_first_seen_order() -> None:
    """Default inference keys each video by its non-date components and preserves first-seen order per group."""
    videos = [
        "M01_2024-01-15.mp4",
        "M02_2024-01-15.mp4",
        "M01_2024-02-20.mp4",
...
def test_group_videos_groups_leading_and_trailing_dates_together() -> None:
    """A leading-date name and a trailing-date name that share an identity land in the same group."""
...
def test_group_videos_paths_with_directories_use_stem_only() -> None:
    """The grouping key is derived from the file stem, ignoring directory and extension."""
...
def test_group_videos_undatable_name_becomes_its_own_group() -> None:
    """A name with no recognizable date falls back to its full stem as its own group key."""
...
def test_group_videos_pattern_with_capturing_group() -> None:
    """A pattern whose first capturing group participates keys the video by that group's text."""
...
def test_group_videos_pattern_without_capturing_group_uses_whole_match() -> None:
    """A pattern with no capturing groups keys the video by the whole matched text (group 0)."""
...
def test_group_videos_pattern_non_participating_optional_group_falls_through_to_whole_match() -> None:
    """A non-participating optional group yields None, so the key falls through to the whole match (group 0)."""
...
def test_group_videos_pattern_no_match_keys_by_full_stem() -> None:
    """A stem the pattern fails to match is keyed by its full stem, forming its own group."""
...
def test_group_videos_pattern_empty_whole_match_falls_back_to_stem() -> None:
    """When the pattern matches an empty span (group 0 is empty), the key falls back to the full stem."""
...
def test_group_videos_invalid_pattern_raises_value_error() -> None:
    """An invalid regular expression is reported as a ValueError chained from the underlying re.error."""
...
def test_infer_group_key_trailing_date_identity_leads() -> None:
    """When the date trails the name, the identity components before it form the key."""
    assert _infer_group_key("M01_2024-01-15") == "M01"


def test_infer_group_key_leading_date_identity_trails() -> None:
    """When the date leads the name, the identity components after it form the key."""
    assert _infer_group_key("2024-01-15_M01") == "M01"


def test_infer_group_key_leading_date_drops_numeric_time_components() -> None:
    """Numeric time components sitting between a leading date and the identity are dropped before keying."""
    assert _infer_group_key("2024-01-15_09-30-00_M01") == "M01"


def test_infer_group_key_normalizes_iso_t_separator() -> None:
    """An ISO 8601 date-time 'T' between digits is normalized so the fused date is exposed and stripped."""
...
def test_group_videos_iso_t_time_groups_with_delimited_date() -> None:
    """End-to-end: an ISO 'T'-separated timestamp is normalized so its video groups with a plainly-dated sibling."""
...
def test_is_month_day_pair_month_then_day() -> None:
    """A month followed by a day is a valid month-day pair."""
    assert _is_month_day_pair("01", "15") is True


def test_is_month_day_pair_day_then_month() -> None:
    """A day followed by a month is a valid month-day pair (reversed order)."""
    assert _is_month_day_pair("15", "01") is True


def test_is_month_day_pair_invalid() -> None:
    """Two components that cannot both be a month/day are not a pair."""
...
def test_is_day_piece() -> None:
    """A one- or two-digit day of month is recognized; an out-of-range value is not."""
...
def test_is_year_piece() -> None:
    """A four-digit year and a bare two-digit year are recognized; a three-digit value is not."""
...
def test_is_compact_date_piece_non_digit_rejected() -> None:
    """A component with non-digit characters is not a compact date."""
    assert _is_compact_date_piece("M01") is False


def test_is_compact_date_piece_year_first_ignores_trailing_time() -> None:
    """A leading YYYYMMDD run qualifies, with any trailing time digits ignored."""
...
def test_is_compact_date_piece_year_last() -> None:
    """An eight-digit DDMMYYYY run whose trailing four digits are a year qualifies."""
    assert _is_compact_date_piece("15012024") is True


def test_is_compact_date_piece_digit_but_not_date_rejected() -> None:
    """An all-digit run that is neither year-first nor year-last is not a compact date."""
...
def test_is_fused_named_date_piece_variants() -> None:
    """Fused named-month dates are recognized in day-first, name-first, and year-first forms."""
...
def test_is_fused_named_date_piece_invalid() -> None:
    """A component with no fused month-name/year date is rejected."""
