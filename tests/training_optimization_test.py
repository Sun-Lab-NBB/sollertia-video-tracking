"""Tests for the training optimization profile resolver and the process-global optimization applier."""
...
def test_multi_gpu_strategy_string_values() -> None:
    """The multi-GPU strategy enum is a string enum with the documented literal values."""
...
def test_module_constants() -> None:
    """The multi-GPU floor and the automatic dataloader-worker ceiling hold their documented values."""
...
def test_use_amp_reflects_dtype_presence() -> None:
    """Mixed precision is on exactly when an autocast dtype is set."""
...
def test_use_ddp_only_for_ddp_strategy() -> None:
    """The DDP flag is true only under the DDP strategy."""
...
def test_world_size_is_gpu_count_under_ddp_else_one() -> None:
    """World size counts every GPU under DDP and collapses to one otherwise."""
...
def test_amp_device_type_maps_device_to_autocast_string() -> None:
    """The autocast device-type string is cuda for cuda and passes other devices through unchanged."""
...
def test_describe_cuda_with_all_extras() -> None:
    """The CUDA summary lists the indices, strategy, precision, workers, pinning, and every enabled extra."""
    profile = _profile(
        device="cuda",
        gpus=(0, 1),
...
def test_describe_cpu_without_extras() -> None:
    """The non-CUDA summary upper-cases the device, reports fp32, and omits the extras suffix when none are enabled."""
...
def test_resolve_multi_gpu_single_gpu_auto_is_silent_single(capsys: pytest.CaptureFixture[str]) -> None:
    """A single GPU under the auto request resolves to single-device training without a warning."""
    assert _resolve_multi_gpu(MultiGpuStrategy.AUTO, (0,)) == MultiGpuStrategy.SINGLE
    assert capsys.readouterr().err == ""


def test_resolve_multi_gpu_single_gpu_ddp_warns_and_falls_back(capsys: pytest.CaptureFixture[str]) -> None:
    """Explicitly requesting DDP against one GPU warns and falls back to single-device training."""
    assert _resolve_multi_gpu(MultiGpuStrategy.DDP, (0,)) == MultiGpuStrategy.SINGLE
...
def test_resolve_multi_gpu_single_gpu_dp_warns_and_falls_back(capsys: pytest.CaptureFixture[str]) -> None:
    """Explicitly requesting DP against one GPU also warns and falls back to single-device training."""
    assert _resolve_multi_gpu(MultiGpuStrategy.DP, (0,)) == MultiGpuStrategy.SINGLE
    assert "Requested 'dp'" in capsys.readouterr().err


def test_resolve_multi_gpu_two_gpus_dp_stays_dp() -> None:
    """Two GPUs with an explicit DP request keep DataParallel."""
    assert _resolve_multi_gpu(MultiGpuStrategy.DP, (0, 1)) == MultiGpuStrategy.DP


def test_resolve_multi_gpu_two_gpus_auto_becomes_ddp() -> None:
    """Two GPUs under the auto request resolve to DDP."""
    assert _resolve_multi_gpu(MultiGpuStrategy.AUTO, (0, 1)) == MultiGpuStrategy.DDP


def test_resolve_multi_gpu_two_gpus_ddp_stays_ddp() -> None:
    """Two GPUs with an explicit DDP request keep DDP."""
    assert _resolve_multi_gpu(MultiGpuStrategy.DDP, (0, 1)) == MultiGpuStrategy.DDP
...
def test_choose_workers_caps_at_ceiling(monkeypatch: pytest.MonkeyPatch) -> None:
    """A single rank on a big box is capped at the automatic worker ceiling."""
...
def test_choose_workers_splits_across_ranks(monkeypatch: pytest.MonkeyPatch) -> None:
    """The usable cores are divided evenly across the training ranks."""
...
def test_choose_workers_floors_at_zero(monkeypatch: pytest.MonkeyPatch) -> None:
    """A machine with fewer cores than the reserve floors the worker count at zero."""
...
def test_choose_workers_handles_unknown_cpu_count(monkeypatch: pytest.MonkeyPatch) -> None:
    """When os.cpu_count returns None the fallback of one core is used, yielding zero workers."""
...
def test_resolve_profile_cpu_defaults(monkeypatch: pytest.MonkeyPatch) -> None:
    """A CPU run disables every CUDA-only optimization, uses zero workers, and restores a held-back thread budget."""
...
def test_resolve_profile_cpu_explicit_worker_count(monkeypatch: pytest.MonkeyPatch) -> None:
    """An explicit non-negative worker count overrides the CPU zero-worker default."""
...
def test_resolve_profile_cuda_single_gpu_ampere(monkeypatch: pytest.MonkeyPatch) -> None:
    """A single Ampere GPU auto-enables bfloat16, TF32, and memory pinning, and picks worker count automatically."""
...
def test_resolve_profile_cuda_fp16_enables_gradient_scaler(monkeypatch: pytest.MonkeyPatch) -> None:
    """Forced float16 on CUDA sets the autocast dtype and requires the gradient scaler."""
    _set_cuda(monkeypatch, 1)
    _set_capabilities(monkeypatch, {0: (8, 0)})
    _set_cpu_count(monkeypatch, 12)
...
def test_resolve_profile_cuda_ddp_world_size(monkeypatch: pytest.MonkeyPatch) -> None:
    """Two GPUs under DDP split the worker budget by the world size rather than treating it as one rank."""
...
def test_resolve_profile_dp_with_amp_warns_and_disables(
    ...
    """DataParallel with mixed precision requested is warned about and reported as full float32."""
    _set_cuda(monkeypatch, 2)
    _set_capabilities(monkeypatch, {0: (8, 0), 1: (8, 0)})
    _set_cpu_count(monkeypatch, 12)
...
def test_resolve_profile_cudnn_benchmark_forced_on_without_fixed_size_warns(
    ...
    """Forcing the cuDNN autotuner on without a detected fixed input size enables it but warns."""
    _set_cuda(monkeypatch, 1)
    _set_capabilities(monkeypatch, {0: (8, 0)})
    _set_cpu_count(monkeypatch, 12)
...
def test_resolve_profile_cudnn_benchmark_auto_with_fixed_size_is_silent(
    ...
    """The autotuner's 'auto' default enables it for a fixed input size without a warning."""
    _set_cuda(monkeypatch, 1)
    _set_capabilities(monkeypatch, {0: (8, 0)})
    _set_cpu_count(monkeypatch, 12)
...
def test_apply_runtime_optimizations_sets_cpu_threads(monkeypatch: pytest.MonkeyPatch) -> None:
    """A profile with a CPU thread budget forwards the backend flags and restores the intra-op thread count."""
...
def test_apply_runtime_optimizations_skips_thread_count_when_none(monkeypatch: pytest.MonkeyPatch) -> None:
    """A profile without a CPU thread budget applies the backend flags but leaves the thread count untouched."""
    forwarded: dict[str, object] = {}
    monkeypatch.setattr(
        optimization,
        "apply_backend_flags",
