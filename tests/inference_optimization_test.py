"""Tests for the inference optimization-profile resolver and the resolved profile's derived views.

These tests drive the real ``resolve_inference_profile``/``apply_runtime_optimizations`` logic on a headless, GPU-free
box by monkeypatching only the torch/psutil capability probes the code consults (``torch.cuda.*`` and
``psutil.cpu_count``). No GPU, network, or DLC runtime is touched.
"""
...
from sollertia_video_tracking.inference import optimization as opt
...
def _patch_cuda(monkeypatch, *, count, capability) -> None:
    ...
    # resolve_target_device / supports_ampere call get_device_capability(device=index); accept any call.
...
def test_use_amp_reflects_amp_dtype_presence() -> None:
    """use_amp is True when an autocast dtype is set and False when it is None."""
...
def test_on_cuda_true_only_for_cuda_device() -> None:
    """on_cuda tracks the base device string exactly."""
...
def test_amp_device_type_maps_each_device() -> None:
    """amp_device_type returns 'cuda' for CUDA and the raw device string otherwise."""
...
def test_total_workers_cuda_multiplies_gpus_by_processes() -> None:
    """On CUDA the worker cap is the device count times the per-device process count."""
...
def test_describe_cpu_without_extras() -> None:
    """The CPU branch shows workers x threads and omits the accelerator suffix when nothing is enabled."""
    profile = _profile(
        device="cpu",
        gpus=(),
        cpu_workers=3,
...
def test_describe_mps_uppercases_device() -> None:
    """The fallback branch uppercases the raw device string (MPS) and reports a single worker."""
    profile = _profile(
        device="mps",
        gpus=(),
        cpu_workers=0,
...
def test_resolve_profile_cuda_auto_defaults(monkeypatch) -> None:
    """On Ampere CUDA the auto defaults enable bf16, tf32, channels-last, and pinning, one process per visible GPU."""
...
def test_resolve_profile_cuda_pre_ampere_disables_tf32_and_amp(monkeypatch) -> None:
    """A pre-Ampere GPU leaves tf32 and auto-AMP off, and the varying-input default leaves the autotuner off."""
...
def test_resolve_profile_warns_on_forced_benchmark_without_fixed_size(monkeypatch, capsys) -> None:
    """Forcing the cuDNN autotuner on without a fixed input size honors the flag but emits a warning."""
...
def test_resolve_profile_cpu_forces_cuda_flags_off_and_sizes_workers(monkeypatch) -> None:
    """On CPU the CUDA-only flags are forced off and the worker/thread budget comes from the physical core count."""
...
def test_resolve_profile_mps(monkeypatch) -> None:
    """An MPS request yields no GPU/CPU parallelism fields and a single-worker profile."""
    monkeypatch.setattr(torch.cuda, "is_available", lambda: False)
...
def test_apply_runtime_optimizations_cpu_restores_thread_count(monkeypatch) -> None:
    """A CPU profile forwards its backend flags and restores the worker's intra-op thread count."""
    backend_calls = {}
    monkeypatch.setattr(opt, "apply_backend_flags", lambda **kw: backend_calls.update(kw))
    thread_calls = []
...
def test_apply_runtime_optimizations_skips_thread_count_when_none(monkeypatch) -> None:
    """A profile without a CPU thread budget (e.g. CUDA) applies backend flags but never sets the thread count."""
    backend_calls = {}
    monkeypatch.setattr(opt, "apply_backend_flags", lambda **kw: backend_calls.update(kw))
    thread_calls = []
    monkeypatch.setattr(torch, "set_num_threads", thread_calls.append)
...
def test_resolve_gpu_processes_honors_explicit_value() -> None:
    """An explicit per-device process count of one or more is returned unchanged."""
...
def test_resolve_gpu_processes_defaults_for_non_positive() -> None:
    """A non-positive request (-1 or 0) collapses to the one-video-per-GPU default."""
...
def test_cpu_parallelism_explicit_threads_derive_workers(monkeypatch) -> None:
    """An explicit thread budget fixes the thread count and derives the worker count from the usable cores."""
    monkeypatch.setattr(psutil, "cpu_count", lambda **_kwargs: 32)
...
def test_cpu_parallelism_explicit_workers_split_usable_cores(monkeypatch) -> None:
    """An explicit worker count with an automatic thread budget splits the usable cores across those workers."""
    monkeypatch.setattr(psutil, "cpu_count", lambda **_kwargs: 32)
...
def test_cpu_parallelism_full_auto_uses_default_thread_block(monkeypatch) -> None:
    """Fully automatic sizing uses the default per-worker thread block and fills the usable cores with workers."""
    monkeypatch.setattr(psutil, "cpu_count", lambda **_kwargs: 32)
...
def test_cpu_parallelism_tiny_machine_clamps_to_single_worker(monkeypatch) -> None:
    """A machine with fewer usable cores than the default block clamps the thread and worker counts to at least one."""
...
def test_cpu_parallelism_falls_back_to_os_cpu_count(monkeypatch) -> None:
    """When psutil cannot report physical cores the code falls back to os.cpu_count for the topology."""
