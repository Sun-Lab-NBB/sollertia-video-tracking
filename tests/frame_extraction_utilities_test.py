...
def _build_purge_project(tmp_path, *, scorer="S", with_video_sets=False):
    ...
    _write_label_table(directory_a / f"CollectedData_{scorer}.h5", ["img0001.png", "img0002.png"])
...
def test_normalize_project_config_rewrites_project_path(tmp_path):
    ...
    result = normalize_project_config(config_path, frames_per_video=-1, error_context="Ctx.")
...
def test_normalize_project_config_sets_frame_count(tmp_path):
    ...
    result = normalize_project_config(config_path, frames_per_video=5, error_context="Ctx.")
...
class _FakePool:
    """A stand-in process pool that runs the worker synchronously in-process, preserving task order."""
...
class _FakeContext:
    """A stand-in spawn context vending the fake manager and pool."""
...
class _FakeMultiprocessing:
    """A stand-in for the module's multiprocessing reference exposing only the get_context the code calls."""
...
class _FakeBar:
    """A stand-in AggregateBar recording its lifecycle so the pinned-extraction loop's bar handling is observable."""
...
def test_prune_removes_only_empty_real_directories(tmp_path, capsys):
    ...
    removed = prune_empty_labeled_data_directories(tmp_path, display_progress=True)
...
def test_machine_label_frame_names_unions_all_tables(tmp_path):
    ...
    _write_label_table(directory / "machinelabels-iter1.h5", ["img0002.png"])
    _write_label_table(directory / "MachineLabelsRefine.h5", ["img0003.png"])

    assert machine_label_frame_names(directory) == {"img0001.png", "img0002.png", "img0003.png"}
...
def test_has_outlier_refinement_data(tmp_path):
    ...
    assert has_outlier_refinement_data(with_iteration) is True

    with_refine = tmp_path / "with_refine"
    with_refine.mkdir()
    (with_refine / "MachineLabelsRefine.h5").touch()
    assert has_outlier_refinement_data(with_refine) is True
...
def test_purge_selected_videos_with_unmatched(tmp_path):
    ...
    summary = purge_labeled_data(config_path, videos=(str(video_a), str(bogus)))
...
def test_summarize_whole_project(tmp_path):
    ...
    pending.mkdir()
    _write_label_table(pending / "machinelabels-iter0.h5", ["img0001.png", "img0002.png"])
    _write_label_table(
        pending / f"CollectedData_{scorer}.h5",
    ...
    refined.mkdir()
    _write_label_table(refined / "machinelabels-iter0.h5", ["img0005.png"])
    _write_label_table(refined / f"CollectedData_{scorer}.h5", ["img0005.png"], finite_names={"img0005.png"})
    _write_label_table(refined / "MachineLabelsRefine.h5", ["img0005.png"], finite_names={"img0005.png"})
    ...
    summary = summarize_refinement_status(config_path)
...
def test_summarize_selected_videos(tmp_path):
    ...
    _write_label_table(pending / "machinelabels-iter0.h5", ["img0001.png"])

    bogus = videos_directory / "nope.mp4"
    summary = summarize_refinement_status(config_path, videos=(str(video_path), str(bogus)))
