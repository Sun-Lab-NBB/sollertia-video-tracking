...
from sollertia_video_tracking.frame_extraction import outlier_pipeline as op
...
def _predictions(n_frames: int = 4, bodyparts: tuple[str, ...] = ("nose",), scorer: str = "S") -> pd.DataFrame:
    ...
    return pd.DataFrame(np.zeros((n_frames, len(columns)), dtype=np.float64), columns=columns)
...
class _FakePool:
    """A synchronous stand-in for a multiprocessing pool that runs ``starmap`` in-process."""
...
class _FakeContext:
    """A spawn-context stand-in whose ``Pool`` runs work synchronously and records each requested process count."""
...
def test_load_sliced_predictions_applies_crop_offset(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(op.auxiliaryfunctions, "load_analyzed_data", lambda **_k: (predictions, None, None, None))
    monkeypatch.setattr(op.auxiliaryfunctions, "load_video_metadata", lambda **_k: metadata)
    configuration = {"start": 0.0, "stop": 1.0, "video_sets": {video: {"crop": "5,100,5,100"}}}

    result = op._load_sliced_predictions(
        video=video,
...
def test_discover_analyzed_videos_keeps_only_videos_with_predictions(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(op.auxiliaryfunctions, "find_analyzed_data", fake_find)
    registered = ["/a/v1.mp4", "/b/v2.mp4", "/c/v3.mp4"]

    result = op._discover_analyzed_videos(registered_videos=registered, scorer="S", tracking_method="")
...
def _detect(monkeypatch: pytest.MonkeyPatch, load_stub, algorithm, **overrides):
    ...
    return op._detect_all_videos(**kwargs)
...
def test_detect_all_videos_uncertain_flags_low_confidence(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    candidates, unanalyzed, errors = _detect(
        monkeypatch, load_stub=lambda **_k: predictions, algorithm=OutlierAlgorithm.UNCERTAIN
    )
...
def test_detect_all_videos_jump_flags_large_step(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    candidates, _unanalyzed, _errors = _detect(
        monkeypatch, load_stub=lambda **_k: predictions, algorithm=OutlierAlgorithm.JUMP, pixel_distance_threshold=5.0
    )
...
def test_detect_all_videos_fitting_reduces_via_pool(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    candidates, unanalyzed, errors = _detect(
        monkeypatch,
        load_stub=lambda **_k: _predictions(n_frames=5),
...
def test_detect_fitting_outliers_auto_workers_reports_progress(
    ...
    monkeypatch.setattr(op.multiprocessing, "get_context", lambda *_a: context)
    monkeypatch.setattr(op, "fit_keypoint_distance", lambda *_a: np.array([0.0, 0.0, 100.0, 0.0, 0.0]))

    result = op._detect_fitting_outliers(
...
def test_count_directory_frames_ignores_overlays(tmp_path: Path) -> None:
    ...
    assert op._count_directory_frames(tmp_path) == 2
...
def _install_extract_stubs(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(op.dlc_outlier_frames, "attempt_to_add_video", object(), raising=False)
...
def test_extract_one_video_writes_frames_and_reports_ok(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(op.dlc_outlier_frames, "ExtractFramesbasedonPreselection", fake_extract)
    ...
    video, written, status = op._extract_one_video(task)

    assert (video, written, status) == ("/v/clip.mp4", 3, "ok")
    assert op._count_directory_frames(tmp_path / "labeled-data" / "clip") == 3
...
def test_extract_one_video_without_progress_queue(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(op.auxiliaryfunctions, "read_config", lambda _p: configuration)
    monkeypatch.setattr(op, "_load_sliced_predictions", lambda **_k: _predictions())
    monkeypatch.setattr(op.dlc_outlier_frames, "ExtractFramesbasedonPreselection", lambda **_k: None)
...
def test_extract_one_video_missing_predictions_is_not_analyzed(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(op, "_load_sliced_predictions", raise_missing)
    ...
    assert op._extract_one_video(task) == ("/v/clip.mp4", 0, "not_analyzed")
...
def test_extract_one_video_captures_extraction_error(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(op.dlc_outlier_frames, "ExtractFramesbasedonPreselection", raise_boom)
...
def test_clear_video_iteration_outliers_keeps_labeled_and_drops_refinement(tmp_path: Path) -> None:
    ...
    _write_labels(directory / "CollectedData_human.h5", ["img0002.png"])  # img0002 is a real human label -> kept.
    ...
    removed, had_refined = op._clear_video_iteration_outliers(directory=directory, iteration=0, scorer="human")
...
def _make_iteration_dir(root: Path, name: str, frame: str, *, refinement: bool = False) -> Path:
    ...
    directory.mkdir(parents=True)
    _write_labels(directory / "machinelabels-iter0.h5", [frame])
...
def _patch_dlc_boundary(monkeypatch: pytest.MonkeyPatch, configuration: dict) -> None:
    ...
    monkeypatch.setattr(op.auxiliaryfunctions, "read_config", lambda _p: configuration)
    monkeypatch.setattr(
        op.auxiliaryfunctions,
        "intersection_of_body_parts_and_ones_given_by_user",
        lambda **_k: ["nose"],
    )
    monkeypatch.setattr(op.auxfun_multianimal, "get_track_method", lambda *_a, **_k: "")
    monkeypatch.setattr(op.auxiliaryfunctions, "get_scorer_name", lambda **_k: ("scorerX", ""))
...
def test_orchestrator_matched_videos_run_extraction(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    config_path = _write_config(tmp_path)
    _patch_dlc_boundary(monkeypatch, _base_config(str(video_path)))
    monkeypatch.setattr(op, "_detect_all_videos", lambda **_k: ({str(video_path): [1, 2, 3]}, [], []))
    sentinel = object()
    captured: dict = {}
...
def test_orchestrator_discovers_videos_when_none_named(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(op, "_discover_analyzed_videos", lambda **_k: [video_path])
    monkeypatch.setattr(op, "_detect_all_videos", lambda **_k: ({video_path: [4, 5]}, [], []))
    sentinel = object()
...
def test_orchestrator_empty_candidates_returns_early_summary(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(op, "_discover_analyzed_videos", lambda **_k: [video_path])
    monkeypatch.setattr(
        op, "_detect_all_videos", lambda **_k: ({video_path: []}, ["/una.mp4"], [("/bad.mp4", "detail")])
    )
...
def test_orchestrator_candidate_step_subsamples(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _patch_dlc_boundary(monkeypatch, _base_config(video_path))
    monkeypatch.setattr(op, "_discover_analyzed_videos", lambda **_k: [video_path])
    monkeypatch.setattr(op, "_detect_all_videos", lambda **_k: ({video_path: [0, 1, 2, 3, 4, 5]}, [], []))
    captured: dict = {}
