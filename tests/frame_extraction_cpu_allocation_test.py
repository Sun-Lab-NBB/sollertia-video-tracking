"""Tests for the CPU-core allocation planner used to pin parallel frame-extraction workers."""
...
def test_automatic_worker_and_core_counts_saturating_blocks() -> None:
    """Automatic worker/core counts give each worker a saturating block, capped at the video count."""
    ...
    assert [len(s) for s in core_sets] == [4, 4, 4]
...
def test_automatic_counts_fill_budget_with_leftover_block() -> None:
    """When videos exceed full blocks and cores do not divide evenly, a smaller leftover block is added."""
    ...
    assert [len(s) for s in core_sets] == [4, 4, 4, 4, 4, 4, 4, 2]
...
def test_automatic_counts_no_leftover_when_budget_divides_evenly() -> None:
    """An evenly divisible budget with surplus videos yields only full blocks and no leftover worker."""
    ...
    assert worker_count == 8
    assert [len(s) for s in core_sets] == [4] * 8


def test_automatic_counts_tiny_machine_single_worker() -> None:
    """A machine too small for even one saturating block collapses to a single worker on all usable cores."""
    worker_count, core_sets = plan_core_allocation(
...
def test_explicit_worker_count_spreads_cores_evenly() -> None:
    """An explicit worker count with automatic cores spreads the usable cores as evenly as possible."""
    ...
    assert sorted((len(s) for s in core_sets), reverse=True) == [8, 8, 7, 7]
    _assert_disjoint_within_usable(core_sets, usable_core_count=30)


def test_explicit_worker_count_capped_at_video_count() -> None:
    """A request for more workers than videos runs one worker per video."""
...
def test_explicit_cores_per_worker_automatic_worker_count() -> None:
    """Explicit cores-per-worker sizes the worker count from the usable cores, capped at the video count."""
    ...
    assert worker_count == 4
    assert [len(s) for s in core_sets] == [5, 5, 5, 5]


def test_explicit_both_counts() -> None:
    """Explicit worker and core counts are honored exactly when they fit the usable cores."""
    worker_count, core_sets = plan_core_allocation(
        video_count=10, total_core_count=32, worker_count=3, cores_per_worker=6, reserved_core_count=2
    )
    assert worker_count == 3
    assert [len(s) for s in core_sets] == [6, 6, 6]


def test_negative_reserved_core_count_treated_as_zero() -> None:
    """A negative reserved-core request is clamped so the whole machine is usable."""
    worker_count, core_sets = plan_core_allocation(
...
def test_oversubscription_raises_value_error() -> None:
    """An explicit configuration that cannot fit the usable cores raises rather than overlapping blocks."""
...
def test_pin_worker_binds_to_claimed_core_set(monkeypatch: pytest.MonkeyPatch) -> None:
    """The worker pins its CPU affinity to the core set it claims from the shared queue."""
...
def test_pin_worker_empty_queue_is_silent() -> None:
    """An empty queue (more workers than slots) degrades to an unpinned worker without raising."""
