...
def test_budget_already_met_when_existing_exceeds_target():
    ...
    plan = plan_video_sampling(
        videos=["a", "b"], extracted_frame_counts={"a": 10, "b": 10}, frames_per_video_count=10, total_frame_budget=5
    )
...
def test_budget_already_met_at_exact_target():
    ...
    plan = plan_video_sampling(
        videos=["a"], extracted_frame_counts={"a": 10}, frames_per_video_count=10, total_frame_budget=10
    )
    assert plan.budget_already_met is True
    assert plan.selected_videos == ()
...
def test_uniform_early_return_in_unextracted_tier():
    ...
    plan = plan_video_sampling(
        videos=["a", "b", "c", "d"], extracted_frame_counts={}, frames_per_video_count=10, total_frame_budget=25
    )
    # a, b, c contribute 30 >= 25. Video d is never reached.
...
def test_uniform_spills_into_below_ceiling_tier_and_completes_loop():
    ...
    plan = plan_video_sampling(
        videos=["a", "b"], extracted_frame_counts={"a": 5}, frames_per_video_count=10, total_frame_budget=20
    )
    # unextracted [b] (cap 10) fills first, then below-ceiling [a] (cap 5) tops the budget up. The loop runs to
    # completion.
...
def test_uniform_target_unreachable_selects_everything():
    ...
    plan = plan_video_sampling(
        videos=["a"], extracted_frame_counts={}, frames_per_video_count=10, total_frame_budget=25
    )
    assert plan.target_unreachable is True
    assert plan.selected_videos == ("a",)
    assert plan.projected_frame_count == 10
...
def test_balanced_single_group_unreachable_exits_when_heap_empties():
    ...
    plan = plan_video_sampling(
        videos=["a"], extracted_frame_counts={}, frames_per_video_count=10, total_frame_budget=100, groups={"g1": ["a"]}
    )
    assert plan.target_unreachable is True
    assert plan.selected_videos == ("a",)
...
def test_balanced_group_exhaustion_does_not_requeue():
    ...
    # g1 (1 video) empties and is not requeued. Group g2 is drained tail-first (c then b).
...
def test_balanced_pins_across_group_membership_states():
    ...
    plan = plan_video_sampling(
        videos=["a", "b", "x"],
        extracted_frame_counts={"b": 10},
        frames_per_video_count=10,
        total_frame_budget=100,
        groups={"g1": ["a", "b"]},
        pinned_videos=("a", "b", "x"),
    )
    # Video a is grouped and available -> removed and counted. Video b is grouped but at the ceiling (not available).
    # Video x is ungrouped.
...
def test_uniform_selection_valid_with_real_random_shuffle(monkeypatch):
    ...
    plan = plan_video_sampling(
        videos=["a", "b", "c", "d", "e"], extracted_frame_counts={}, frames_per_video_count=10, total_frame_budget=30
    )
...
class _NoShuffleRandom:
    """Replaces Random with a no-op shuffle, making tier and group ordering deterministic in tests."""
