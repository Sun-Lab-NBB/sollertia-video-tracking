...
from sollertia_video_tracking.frame_extraction import outlier_detection
...
def test_uncertain_flags_frames_with_any_low_confidence_keypoint() -> None:
    ...
    assert uncertain_outlier_indices(predictions=predictions, minimum_confidence=0.5) == [11, 13]
...
def test_uncertain_returns_empty_when_all_confident() -> None:
    ...
    predictions = _make_predictions(
        per_bodypart={"bp0": ([0, 0], [0, 0], [0.95, 0.99]), "bp1": ([1, 1], [1, 1], [0.9, 0.9])},
    )
    assert uncertain_outlier_indices(predictions=predictions, minimum_confidence=0.5) == []
...
def test_jump_flags_over_threshold_displacement_and_never_first_frame() -> None:
    ...
    assert jump_outlier_indices(predictions=predictions, pixel_distance_threshold=5.0) == [2]
...
def test_jump_ignores_likelihood_channel() -> None:
    ...
    assert jump_outlier_indices(predictions=predictions, pixel_distance_threshold=1.0) == []
...
def test_jump_sums_displacement_across_individuals_sharing_a_bodypart() -> None:
    ...
    assert jump_outlier_indices(predictions=predictions, pixel_distance_threshold=4.0) == [2]
...
def test_fit_keypoint_distance_returns_euclidean_deviation_from_fit(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(outlier_detection, "FitSARIMAXModel", _fake_fit)
    ...
    np.testing.assert_allclose(actual=deviation, desired=np.full(3, np.sqrt(2.0)))
    ...
    for call in recorded_calls:
        ...
        assert call["alpha"] == outlier_detection._DISCARDED_INTERVAL_ALPHA
...
def test_fit_keypoint_distance_returns_all_nan_when_fit_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(outlier_detection, "FitSARIMAXModel", _raising_fit)
...
def test_fitting_outlier_indices_threshold_branch() -> None:
    ...
    assert fitting_outlier_indices(
        keypoint_deviations=deviations, frames_per_video_count=2, pixel_distance_threshold=5.0
    ) == [1, 3]
...
def test_fitting_outlier_indices_returns_all_qualifiers_without_truncation() -> None:
    ...
    flagged = fitting_outlier_indices(
        keypoint_deviations=deviations, frames_per_video_count=1, pixel_distance_threshold=1.0
    )
...
def test_fitting_outlier_indices_fallback_takes_most_deviant() -> None:
    ...
    assert fitting_outlier_indices(
        keypoint_deviations=deviations, frames_per_video_count=1, pixel_distance_threshold=1000.0
    ) == [1, 3]
...
def test_fitting_outlier_indices_all_skipped_keypoints_frame_is_nan_and_unflagged() -> None:
    ...
    assert fitting_outlier_indices(
        keypoint_deviations=deviations, frames_per_video_count=2, pixel_distance_threshold=1.0
    ) == [1]
